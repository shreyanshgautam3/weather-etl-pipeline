...
def init_db(engine):
    """Initializes the Silver layer weather logs table."""
    create_table_query = """
...
def insert_weather_data(engine, record: WeatherData):
    """
    SILVER LAYER LOAD:
    Executes an idempotent insertion strategy using a SQL CTE.
    Skips records with identical city + timestamp combinations.
    """
    cte_insert_query = """
    WITH new_weather AS (
        SELECT :city AS city,
